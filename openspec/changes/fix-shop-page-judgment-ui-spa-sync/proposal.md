...
- SPAのルート変更（`history.pushState`/`replaceState`/`popstate`、およびそれらで捕捉できないDOM差し替え）を検知する仕組みを追加する。
- ルート変更のたびに、現在のURLがショップページ (`/shop/menu/{shopId}`) にマッチするかを再評価し、判定パネルの表示/非表示を同期させる。
...
- `src/adapters/ShopPageAdapter.ts`: 変更なし（`/shop/menu/{shopId}`のみを対象とする現行の判定はそのまま利用する）。
- `src/managers/JudgmentManager.ts`: 変更なし想定（`mountBadge`/`createControls`は再利用する）。ただしパネルの複数回生成・除去に対応できるか確認が必要。
